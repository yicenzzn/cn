...
​		配置云防火墙防护端点的IP和路由用于将云防火墙处理后的流量进行转发（发送到其实际目的端），此步骤可以通过页面方式也可以通过命令行方式来进行配置，以下介绍通过web页面方式配置步骤，命令行配置步骤参见**[云防火墙-命令行方式配置防护端点](https://docs.jdcloud.com/cn/cloudfw/03-fwcli)** 。
