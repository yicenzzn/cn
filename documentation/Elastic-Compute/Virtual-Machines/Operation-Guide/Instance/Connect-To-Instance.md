...
创建实例后，您可以在控制台使用京东云提供的连接工具或使用其他第三方客户端工具连接实例进行用户登录。京东云目前提供两种实例连接工具：VNC及Webterminal，您可根据使用场景选择合适的连接方式。
...
> 登录密码：创建实例时如果您选择自动生成密码，系统将以短信及邮件方式发送给您，您也可以选择自定义密码。如果忘记密码，您可以在实例创建后通过 重置密码 功能重新设置。

## 不同连接主要对比如下：
| 对比项               | WebTerminal          | VNC          | 第三方客户端
| :------------------- | :------------------- |:---------------|:---------------|
| 实例操作系统要求  | Linux、Windows | Linux、Windows | 视不同工具支持情况
